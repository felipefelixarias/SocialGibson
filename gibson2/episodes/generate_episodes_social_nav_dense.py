        ...
        for scene_id in dataset_split[split]:
                ...
                for i in range(env.task.num_pedestrians):
                    ...
                    for step in range(episode_length):
                        ...
                        episode_info['pedestrians'][i]['target_pos'][step] = \
                            list(target_pos)
                    episode_info['pedestrians'][i]['path'] = []
            env.close()
            ...
            for i in range(raw_num_episodes):
                ...
                shortest_path, _ = env.scene.get_shortest_path(
                    ...
                    entire_path=True)
                episode_info = episode_config.episodes[i]
                ...
                for step in range(episode_length):
                    ...
                    #Save paths for all pedestrians
                    for idx, agent_no in enumerate(env.task.orca_pedestrians):
                        ped_pos = env.task.orca_sim.getAgentPosition(agent_no)
                        if(isnan(ped_pos[0]) or isnan(ped_pos[1])):
                            break
                        episode_info['pedestrians'][idx]['path'].append([ped_pos[0], ped_pos[1], 0.0])
